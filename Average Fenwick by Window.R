...
averages <- filtered_data %>%
  summarise(
    avg_away_at_fight = mean(away_fenwick_at_fight, na.rm = TRUE),
    avg_home_at_fight = mean(home_fenwick_at_fight, na.rm = TRUE),
    avg_away_after_2 = mean(away_fenwick_after_2, na.rm = TRUE),
    avg_home_after_2 = mean(home_fenwick_after_2, na.rm = TRUE),
    avg_away_after_5 = mean(away_fenwick_after_5, na.rm = TRUE),
    avg_home_after_5 = mean(home_fenwick_after_5, na.rm = TRUE),
    avg_away_after_10 = mean(away_fenwick_after_10, na.rm = TRUE),
    avg_home_after_10 = mean(home_fenwick_after_10, na.rm = TRUE),
    avg_away_until_eop = mean(away_fenwick_until_eop, na.rm = TRUE),
    avg_home_until_eop = mean(home_fenwick_until_eop, na.rm = TRUE),
    avg_away_until_eog = mean(away_fenwick_until_eog, na.rm = TRUE),
    avg_home_until_eog = mean(home_fenwick_until_eog, na.rm = TRUE),
    avg_away_until_next_fight = mean(away_fenwick_until_next_fight, na.rm = TRUE),
    avg_home_until_next_fight = mean(home_fenwick_until_next_fight, na.rm = TRUE)
  )
...
plot_data_long <- plot_data %>%
  pivot_longer(cols = c(Away, Home), names_to = "Team", values_to = "Average_Fenwick")
...
ggplot(plot_data_long, aes(x = Situation, y = Average_Fenwick, color = Team, group = Team)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  theme_minimal() +
  labs(title = "Average Fenwick in Different Situations",
       x = "Situation",
       y = "Average Fenwick",
       color = "Team") +
